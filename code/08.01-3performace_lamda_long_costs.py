...
PORTFOLIO_RETURNS_PATH = r"C:\Users\vilke\OneDrive\Dokumentai\thesis_project\output\portfolios\portfolio_returns_lambda_long_only.csv"
...
OUTPUT_PATH = OUTPUT_DIR / "portfolio_performance_lambda_long_only_after_costs.csv"
...
# ============================================================
# 3. Clean grouping columns
# ============================================================

weights["base_model"] = weights["base_model"].astype(str)
weights["lambda"] = weights["lambda"].astype(str)

returns["base_model"] = returns["base_model"].astype(str)
returns["lambda"] = returns["lambda"].astype(str)
...
for keys, group in weights.groupby(group_cols, dropna=False):

    model, base_model, lam = keys
    ...
    for current_date in sorted(group["date"].unique()):
        ...
        if len(missing_assets) > 0:
            print(
                ...
                f"model={model}, lambda={lam}, missing={missing_assets}"
            )
            continue
...
results_data = returns.merge(
    turnover,
    on=["date", "model", "base_model", "lambda"],
    how="left"
)
...
for keys, df in results_data.groupby(group_cols, dropna=False):
...
print("After-cost lambda long-only portfolio performance saved.")
print(results_df)
